...
async function getLearnerProgress(userId) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(learnerProgress).where(eq(learnerProgress.userId, userId));
}
async function setLearnerProgress(userId, moduleId, completed) {
  const db = await getDb();
  ...
  await db.insert(learnerProgress).values({ userId, moduleId }).onDuplicateKeyUpdate({ set: { completedAt: /* @__PURE__ */ new Date() } });
...
async function createLearnerFile(input) {
  const db = await getDb();
  if (!db) throw new Error("Database is not available");
...
async function createLearnerSubmission(input) {
  const db = await getDb();
  if (!db) throw new Error("Database is not available");
...
function isAllowedUpload(contentType, sizeBytes) {
...
var learnerSubmissionInput = z2.object({
  submissionType: z2.enum(["project", "resource"]),
  title: z2.string().trim().min(5, "Add a descriptive title.").max(255),
  description: z2.string().trim().min(20, "Share enough detail for a reviewer to understand the suggestion.").max(4e3),
  url: z2.string().trim().url("Use a full URL beginning with https://.").max(1024).optional().or(z2.literal("")),
  moduleId: z2.string().trim().max(64).optional().or(z2.literal(""))
});
...
var appRouter = router({
  ...
  roadmap: router({
    ...
    setProgress: protectedProcedure.input(z3.object({ moduleId: z3.string().min(3).max(64), completed: z3.boolean() })).mutation(({ ctx, input }) => setLearnerProgress(ctx.user.id, input.moduleId, input.completed))
  }),
  resources: router({
  ...
  files: router({
    ...
    upload: protectedProcedure.input(z3.object({
      ...
      contentType: z3.string().min(3).max(255),
      dataBase64: z3.string().min(4).max(112e5)
  ...
  submissions: router({
    create: protectedProcedure.input(learnerSubmissionInput).mutation(({ ctx, input }) => createLearnerSubmission({
      userId: ctx.user.id,
