...
| Public Ryan & Matt search and oEmbed Sub-Workflows verification | 1 additional exact-title record | `docs/verified-youtube-search-oembed-ryan-matt-subworkflows-2026-08-25.json` |
| Public Ryan & Matt travel-safety no-match verification | No change; exact title and creator not found | `docs/rejected-youtube-candidate-ryan-matt-travel-safety-title-variant-2026-08-25.json` |
| Public Arabic data-types provider-mismatch verification | No change; public creator does not match stored provider | `docs/rejected-youtube-candidate-provider-mismatch-arabic-data-types-2026-08-25.json` |
| Public Arabic automation-series provider-mismatch verification | No change; public creators do not match stored provider | `docs/rejected-youtube-candidate-provider-mismatch-arabic-series-2026-08-25.json` |
| Total catalogue status | 185 direct YouTube pages; 21 search fallbacks remain | Persistent-library verification query |

The recovered entry **`016 | AI Agents Introduction | مقدمة عن وكلاء الذكاء الاصطناعي`** was checked against its public video page before replacement. Its title matched exactly and its creator metadata identified Karim Nabil; the persistent provider and description were corrected to match that evidence. The remaining 21 catalogue search links have not been guessed or bulk-replaced; each still requires an exact item-level title-and-provider match from a public playlist, direct page, or Notebook export. In the DeepSeek batch, a candidate whose imported title used **`in n8n`** while public oEmbed used **`في n8n`** was deliberately kept as a search fallback rather than being treated as an exact match. The later hosting-part-one and hosting-part-two candidates used a narrow nonbreaking space before **`n8n`**, so they were likewise retained as search fallbacks.
...
Public YouTube search results exposed the exact canonical Sub-Workflows and Parallel Executions tutorial page for `150170`. Its oEmbed title and author match the stored record at the code-point level; the guarded update is recorded in `docs/verified-youtube-search-oembed-ryan-matt-subworkflows-2026-08-25.json`.

The remaining travel-safety fallback `150140` was retained after public YouTube and web search produced no exact Ryan & Matt Data Science title-and-creator match. The nearest public result was a different DATA JARVIS lesson, so no canonical page was inferred; the no-op evidence is recorded in `docs/rejected-youtube-candidate-ryan-matt-travel-safety-title-variant-2026-08-25.json`.

The Arabic data-types fallback `150107` was also retained: its public YouTube result attributes the lesson to Karim Nabil, not to the stored Ryan & Matt Data Science provider. No direct page was accepted under the provider-correspondence requirement; the no-op evidence is recorded in `docs/rejected-youtube-candidate-provider-mismatch-arabic-data-types-2026-08-25.json`.

The four remaining Arabic entries labelled Ryan & Matt (`150114`–`150117`) were also retained after their public search results identified Kariim Nabiil or AI Plus as the creators. This recovery pass does not silently correct imported provider metadata, so their transparent search fallbacks remain in place; the no-op evidence is recorded in `docs/rejected-youtube-candidate-provider-mismatch-arabic-series-2026-08-25.json`.
