...
| Total catalogue status | 160 direct YouTube pages; 46 search fallbacks remain | Persistent-library verification query |

The recovered entry **`016 | AI Agents Introduction | مقدمة عن وكلاء الذكاء الاصطناعي`** was checked against its public video page before replacement. Its title matched exactly and its creator metadata identified Karim Nabil; the persistent provider and description were corrected to match that evidence. The remaining 46 catalogue search links have not been guessed or bulk-replaced; each still requires an exact item-level title-and-provider match from a public playlist, direct page, or Notebook export. In the DeepSeek batch, a candidate whose imported title used **`in n8n`** while public oEmbed used **`في n8n`** was deliberately kept as a search fallback rather than being treated as an exact match. The later hosting-part-one candidate used a narrow nonbreaking space before **`n8n`**, so it was likewise retained as a search fallback.
