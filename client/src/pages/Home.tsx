...
export default function Home() {
  ...
  const filteredResources = useMemo(() => filterLearningResources(resourceList, resourceFilter, resourceQuery), [resourceFilter, resourceList, resourceQuery]);
      ...
      <section id="library" className="paper-grain scroll-mt-16 bg-white text-[#15151a]">
        <div className="mx-auto max-w-[1480px] px-5 py-20 sm:px-8 lg:px-10 lg:py-28">
          <div className="grid gap-8 border-b border-black/10 pb-8 lg:grid-cols-[.9fr_1.1fr] lg:items-end">
            ...
            <div><p className="max-w-xl text-sm leading-7 text-[#606065]">Official n8n, MDN, OpenAI, Arabic and English n8n Mastery videos, YouTube, and Notebook material support the whole roadmap, from automation mechanics to production hardening.</p><label className="relative mt-5 block max-w-xl"><BookOpen className="pointer-events-none absolute left-4 top-1/2 size-4 -translate-y-1/2 text-[#c92f55]" /><input value={resourceQuery} onChange={(event) => setResourceQuery(event.target.value)} placeholder="Search topics, providers, or sources" className="w-full rounded-full border border-black/15 bg-white py-3 pl-11 pr-4 text-sm outline-none transition-colors placeholder:text-[#85858a] focus:border-[#ea4b71]" /></label></div>
          </div>
